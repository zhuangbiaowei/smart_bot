...
SmartBot::Skill.register :search do
  ...
  register_tool :smart_search do
    ...
    tool_proc do
      ...
      # 尝试 MCP 搜索
      mcp_server = SearchSkillHelpers.find_mcp_server(:search)
      if mcp_server
        result = SearchSkillHelpers.call_mcp_tool(mcp_server, "search", { "query" => query })
        next SearchSkillHelpers.format_mcp_result(result, query) if result
      end
      
      # 回退到 SerpAPI
      if ENV["SERP_API_KEY"]
        result = SearchSkillHelpers.call_native_tool(:serp_search, { "query" => query, "count" => count })
        next result if result && !result[:error]
      end
      
      # 回退到 Brave Search
      if ENV["BRAVE_API_KEY"]
        result = SearchSkillHelpers.call_native_tool(:web_search, { "query" => query, "count" => count })
        next result if result && !result[:error]
      end
  ...
  register_tool :smart_scrape do
    ...
    tool_proc do
      ...
      # 尝试 MCP scrape
      mcp_server = SearchSkillHelpers.find_mcp_server(:scrape)
      if mcp_server
        result = SearchSkillHelpers.call_mcp_tool(mcp_server, "scrape", { "url" => url })
        next { content: result["content"] } if result && result["content"]
      end
      
      # 回退到本地 web_fetch
      SearchSkillHelpers.call_native_tool(:web_fetch, { "url" => url, "extract_mode" => "markdown" })
    end
  end
...
# 辅助方法模块
module SearchSkillHelpers
  def self.find_mcp_server(tool_name)
  ...
  def self.call_mcp_tool(server_name, tool_name, params)
  ...
  def self.call_native_tool(tool_name, params)
    tool = SmartAgent::Tool.find_tool(tool_name)
    tool&.call(params)
  end

  def self.format_mcp_result(result, query)
